...
@app.route('/create_illness', methods=['GET', 'POST'])
@login_required
def create_illness():
    error = 'Wrong user role'
    if current_user.role == 1:
        new_illness = Illness(request.form['name'], request.form['type'], request.form['description'],
                              request.form['status'],
                              current_user.id, request.form['price'])
        if illness_services.add_request(new_illness):
            error = None
        else:
            error = 'Invalid data'
    return render_template('create_illness.html', error=error)
...
def choose_illness():
    ...
    if current_user.role == 2:  # admin
        ...
        if not data:
            error = 'No accepted illness so far'
    else:
        data = illness_services.show_by_status('accepted')
    return render_template('illness.html', data=data, error=error)
...
def process_choice():
    ...
    if current_user.role == 3:  # doctor
        for id in selected_rows:
            illness_services.update_status(id, 'in-process')
            feedback_serv.take_request(Feedback(illnes_id=id, doctor_id=current_user.id))
...
def my_requests():
    ...
    if current_user.role == 3:
        illness_id = feedback_serv.get_illness_id(current_user.id)
        for id in illness_id:
...
def make_feedback():
    error = None
    data = None
    if current_user.role == 1:  # get completed illness by current user without feedback
        data = user_services.get_no_feedback(current_user.id)
        if data is None:
            error = 'No completed illness'
